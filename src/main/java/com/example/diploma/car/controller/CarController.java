...
public class CarController {
    ...
    @PutMapping("/{uuid}")
    public ResponseEntity<UUID> updateCar(
            @PathVariable UUID uuid,
            @Valid @RequestPart("payload") CreateCarRequest createCarRequest,
            @RequestPart(value = "image", required = false) MultipartFile image
    ) {
        return ResponseEntity.ok(createCarService.updateCar(uuid, createCarRequest, image).getUuid());
    }

    @PatchMapping("/{uuid}")
    public ResponseEntity<UUID> patchCar(
            @PathVariable UUID uuid,
            @Valid @RequestPart("payload") CreateCarRequest createCarRequest,
            @RequestPart(value = "image", required = false) MultipartFile image
    ) {
    ...
    public ResponseEntity<Void> deleteCar(@PathVariable UUID uuid) {
        createCarService.deleteCar(uuid);
